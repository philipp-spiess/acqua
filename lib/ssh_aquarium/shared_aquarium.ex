defmodule SshAquarium.SharedAquarium do
  ...
  defstruct [
    # MapSet of viewer PIDs
    :viewers,
    # Map of fish_id -> fish_state
    :fish,
    # Terminal dimensions and cell size
    :terminal_config,
  ...
  def update_terminal_config(pid, term_columns, term_rows, cell_width, cell_height) do
    GenServer.cast(
      pid,
      {:update_terminal_config, term_columns, term_rows, cell_width, cell_height}
    )
  end
  ...
  def handle_call({:add_viewer, viewer_pid}, _from, state) do
    ...
    state =
      if MapSet.size(state.viewers) == 0 do
        ...
        # Hide cursor
        send(viewer_pid, {:aquarium_broadcast, "\x1b[?25l"})
        # Enable mouse click reporting
        send(viewer_pid, {:aquarium_broadcast, "\x1b[?1000h"})
        ...
        %{
          state
          | # Mark as detecting
            terminal_config: :detecting,
            # Will be set when terminal is detected
            fish_counter: 0
        }
      else
        ...
        if is_map(state.terminal_config) do
          ...
          %{state | fish: fish, fish_counter: state.fish_counter + @fish_per_connection}
        else
    ...
    state = %{state | viewers: new_viewers, connection_counter: connection_id}
  ...
  def handle_cast({:remove_viewer, connection_id}, state) do
    ...
    Logger.info(
      "Connection #{connection_id} left. Remaining viewers: #{MapSet.size(new_viewers)}"
    )
    ...
    state =
      if MapSet.size(new_viewers) == 0 and state.animation_timer do
        ...
        %{state | animation_timer: nil, fish: %{}, terminal_config: nil, fish_counter: 0}
      else
        state
      end
  ...
  def handle_cast(
    ...
    cond do
      state.terminal_config == :detecting ->
        ...
        Logger.info(
          "  - Total canvas: #{term_columns * cell_width}x#{term_rows * cell_height} pixels"
        )
  ...
  def handle_info(:animate, state) do
    if MapSet.size(state.viewers) > 0 and is_map(state.terminal_config) do
      ...
      if output != "" do
        Logger.debug(
          "Broadcasting animation frame with #{byte_size(output)} bytes to #{MapSet.size(state.viewers)} viewers"
        )
  ...
  defp add_fish_for_connection(existing_fish, terminal_config, connection_id, count, fish_counter) do
    ...
    new_fish =
      for i <- 1..count, into: %{} do
        ...
        if i <= 5 do
          Logger.debug(
            "Fish ##{fish_id} initial position: (#{Float.round(px, 2)}, #{Float.round(py, 2)})"
          )
        end
  ...
  defp animate_fish(fish_map, terminal_config) do
    ...
    {new_fish, outputs} =
      Enum.map_reduce(fish_map, [], fn {fish_id, fish}, acc_outputs ->
        ...
        direction_change_output =
          SshAquarium.KittyGraphics.handle_direction_change(
            updated_fish,
            fish.dx,
            terminal_config
          )
    ...
    if button == 0 do
      ...
      {updated_fish_map, found_collision} =
        Enum.reduce(fish_map, {fish_map, false}, fn {fish_id, fish}, {acc, collision_found} ->
          Logger.debug(
            "Checking fish ##{fish.id} at (#{fish.px}, #{fish.py}) owned by #{fish.owner_id}"
          )
          ...
          if mouse_x >= fish.px and mouse_x <= fish.px + 64 and
            ...
            if fish.owner_id == connection_id do
              Logger.debug(
                "ALLOWED: Connection #{connection_id} clicking their own fish ##{fish.id}"
              )
              ...
              new_bubbles =
                for i <- 0..2 do
                  %{
                    ...
                    x: fish.px + 32 + (:rand.uniform() - 0.5) * 20,
              ...
              Logger.debug(
                "BLOCKED: Connection #{connection_id} tried to click fish ##{fish.id} owned by #{fish.owner_id}"
              )
